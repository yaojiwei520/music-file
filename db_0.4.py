...
DB_CONFIG = {
    ...
    'database': 'music_data',  # !!! 替换为你的数据库名 (数据库名不变)
    'table_name': 'linjunjie_tracks', # !!! 新增：这里是你修改后的表名
    'charset': 'utf8mb4',
...
class MusicDatabaseManager:
    ...
    def __init__(self, db_config):
        ...
        # 从配置中获取表名，如果未提供则使用默认值 'music_tracks'
        self.table_name = db_config.get('table_name', 'music_tracks')
        self.conn = None
    ...
    def _get_connection(self):
        ...
        if self.conn is None or not self.conn.open:
            print(
                f"尝试连接数据库: {self.db_config['host']}:{self.db_config['port']}/{self.db_config['database']} (表名: {self.table_name}) with user {self.db_config['user']}...")
            try:
...
if __name__ == "__main__":
    ...
    print(f"其中 {total_newly_inserted} 条数据被新插入或更新到数据库 (表名: {db_manager.table_name})。")
    print("----------------------------\n")
