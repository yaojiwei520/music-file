...
FFMPEG_AVAILABLE = False  # 全局标志，用于存储 FFmpeg 的可用性
...
# --- FFmpeg 检查与转换功能（从代码 2 整合） ---

def check_ffmpeg_available():
    """检查系统是否安装了 FFmpeg 并返回其可用性。"""
...
def convert_aac_to_mp3(input_filepath):
    """
    将指定的 AAC 文件转换为 MP3 格式，并在成功后删除原文件。

    Args:
        input_filepath (Path): 待转换的 AAC 文件的完整路径。
    """
    if not FFMPEG_AVAILABLE:
        print_status("  [转换] 警告: FFmpeg 不可用，跳过 AAC 到 MP3 转换。")
        return

    # 1. 检查文件扩展名
    if input_filepath.suffix.lower() != '.aac':
        return  # 不是 aac 文件，直接返回

    # 2. 构造输出路径
    name_without_ext = input_filepath.stem  # 文件的名称部分（不含扩展名）
    output_filepath = input_filepath.with_suffix(".mp3")  # 替换扩展名为 .mp3

    print_status(f"  [转换] 检测到 AAC 文件，开始转换为 MP3: {input_filepath.name}...")
    ...
    command = [
        ...
        "-c:a", "libmp3lame",
        "-qscale:a", "2",  # 较高的 VBR 质量（较低的数字表示较高的质量）
        str(output_filepath)
    ]

    try:
        # 执行 FFmpeg 命令
        subprocess.run(command, check=True, capture_output=True, text=True, encoding='utf-8', timeout=60)
        print_status(f"  [转换] √ 成功转换为: {output_filepath.name}")

        # 4. 删除原 AAC 文件
        try:
            os.remove(input_filepath)
            print_status(f"  [转换] √ 成功删除原 AAC 文件: {input_filepath.name}")
        except OSError as e:
            print_status(f"  [转换] 警告: 无法删除原 AAC 文件: {e}")

    except subprocess.CalledProcessError as e:
        print_status(f"  [转换] × 转换失败！文件: {input_filepath.name}")
        print_status(f"  [转换] FFmpeg 错误信息 (stderr):\n{e.stderr[:500]}...")
    except subprocess.TimeoutExpired:
        print_status(f"  [转换] × 转换超时失败！文件: {input_filepath.name}")
    except Exception as e:
        print_status(f"  [转换] × 转换时发生未知错误: {e}")
...
def download_music_file(url, title, artist):
    ...
    成功下载后，会检查文件是否为 .aac，如果是则自动转换为 .mp3。
    """
    if not url:
        return False
    ...
    # **重要：如果下载的格式是 AAC，这里需要保留 .aac 扩展名**
    extension = extension_match.group(1).lower() if extension_match else 'mp3'

    sanitized_title = sanitize_filename(title)
    sanitized_artist = sanitize_filename(artist)
    # 在下载前，我们先使用 URL 中提供的扩展名
    filename = f"{sanitized_title} - {sanitized_artist}.{extension}"

    # 核心修改：创建目录并构造最终路径
    DOWNLOAD_DIR.mkdir(exist_ok=True)  # 确保 downloads 目录存在
    file_path = DOWNLOAD_DIR / filename  # 完整的下载路径

    # 检查文件是否已存在 (MP3/AAC 文件都算作存在，避免重复下载)
    if file_path.exists():
        print_status(f"  [下载] 文件已存在，跳过下载: {file_path.name}")
        # 如果已存在的是 AAC，仍然尝试转换
        if file_path.suffix.lower() == '.aac' and FFMPEG_AVAILABLE:
            convert_aac_to_mp3(file_path)
        return True  # 文件已存在
    ...
    try:
        ...
        print_status(f" --> 成功保存为: {file_path.name}", end='\n')
...
# --- 歌曲信息获取函数 (保持不变) ---
# ... (search_songs, _get_play_id_from_html, get_music_url 函数保持原样) ...
# 为了代码简洁，这里省略了未修改的三个函数，但在实际运行时，你需要将它们完整地放在这里。
...
if __name__ == "__main__":
    ...
    if check_ffmpeg_available():
        ...
        print_status("【格式转换】警告: FFmpeg 未安装或不可用。AAC 文件将保留原格式。")
        print_status("如果您需要自动转换功能，请先安装 FFmpeg 并将其添加到系统 PATH 中。")
    print_status("-" * 20)
    ...
    if not search_query:
        ...
        if not found_songs:
            ...
            if full_music_results:
                ...
                if FFMPEG_AVAILABLE:
                    print_status("注意：所有下载的 AAC 文件已自动转换为 MP3 格式。")
            else:
