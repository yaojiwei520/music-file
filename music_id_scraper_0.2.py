...
class MyFreeMp3Scraper:
    ...
    def search_music(self, keyword, music_type="netease"):
        ...
        Args:
            keyword (str): 要搜索的音乐关键词（歌曲名或歌手名，可以是组合关键词）。
...
if __name__ == "__main__":
    ...
    # 处理空输入
    if not user_input:
        print("输入为空，将为您搜索默认歌曲 '唯一-光良'。")
        user_input = "唯一-光良"  # 更改默认搜索词以符合模糊搜索的示例

    final_search_keyword = user_input  # 默认的搜索关键词
    ...
    if '-' in user_input:
        ...
        if len(parts) == 2:
            ...
            if part1 and part2:
                # 组合成一个更精确的搜索关键词
                final_search_keyword = f"{part1} {part2}"
                print(f"检测到模糊搜索格式 '{user_input}'。将使用组合关键词 '{final_search_keyword}' 进行搜索。")
            else:
                # 如果某部分为空，则按原始输入搜索
                print(f"检测到模糊搜索格式 '{user_input}'，但部分内容为空。将按原始输入 '{user_input}' 进行搜索。")
                final_search_keyword = user_input
        else:
            # 如果rsplit没有得到两部分（理论上不会，但以防万一）
            print(f"检测到模糊搜索格式 '{user_input}'，但解析失败。将按原始输入 '{user_input}' 进行搜索。")
            final_search_keyword = user_input
    # 否则，按普通关键词搜索，final_search_keyword 保持不变

    # 执行搜索
    music_result = scraper.search_music(final_search_keyword)

    if music_result:
        # 直接打印找到的第一首歌的ID
        print(f"\n找到歌曲ID: {music_result['songid']}")
    else:
        print(f"未找到 '{user_input}' 的相关歌曲。")
