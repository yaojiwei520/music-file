...
if __name__ == "__main__":
    ...
    while True:
        # 2. 搜索关键字
        search_query = input("请输入您想搜索的音乐或歌手 (输入 2 退出): ")

        if search_query == '2':
            print("感谢使用，程序已退出。")
            break  # 用户输入'2'，跳出循环，结束程序

        if not search_query.strip(): # 使用 .strip() 移除空白符，避免只输入空格的情况
            print("输入为空，将为您搜索默认歌曲 '阿杜'。")
            search_query = "阿杜"
        ...
        if music_results:
            ...
            # 3. 选择音乐并下载
            try:
                choice_input = input("请输入您想下载的歌曲序号 (输入 0 放弃下载): ")
                if choice_input.isdigit(): # 确保输入的是数字
                    ...
                    if 1 <= choice <= len(music_results):
                        ...
                        # 优先使用320kbps链接，如果不存在则退回到128kbps
                        download_url = selected_music.get('url_320') or selected_music.get('url_128')

                        download_path = scraper.download_music(download_url, filename)
                        if download_path:
                ...
                print("输入无效，请输入数字。") # 额外的防护，虽然isdigit已处理大部分
            except Exception as e:
                print(f"处理下载选择时发生错误: {e}")
        else:
            print("未找到相关音乐。请尝试其他关键词。")
